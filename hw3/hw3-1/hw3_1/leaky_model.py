...
class Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(self.conv1(x)) #BATCH_SIZE*128*32*32
        #x = self.upsample(x)    #BATCH_SIZE*128*64*64
        x = F.leaky_relu(self.conv2(x)) #BATCH_SIZE*64*64*64
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(self.conv1(x)) #64*64*32
        x = F.leaky_relu(self.conv2(x)) #64*64*64
        x = F.leaky_relu(self.conv3(x)) #64*64*128
        x = F.leaky_relu(self.conv4(x)) #64*64*256
