...
def update_d(iterator, generator, discriminator, d_optimizer):
    ...
    for i,batch in enumerate(iterator):
        ...
        valid = 0.95*torch.ones(batch_size,1).to(device) + 0.05*randn(batch_size, 1).to(device)
        fake = 0.05*torch.ones(batch_size,1).to(device) + 0.05*randn(batch_size, 1).to(device)
        print(valid)
        print(fake)
        noise = randn(batch_size, 100).to(device)
...
def update(iterator, generator, discriminator, optimizer_D, optimizer_G):
    ...
    for i, batch in enumerate(iterator):
        ...
        valid = 0.9*torch.ones(batch_size,1, requires_grad=False).to(device) + 0.1*random.random()*torch.ones(batch_size, 1, requires_grad=False).to(device)
        fake = 0.1*random.random()*torch.ones(batch_size, 1, requires_grad=False).to(device)
