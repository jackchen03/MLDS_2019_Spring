...
class QNet(nn.Module):
    def __init__(self, output_size):
        ...
        self.advantage = nn.Linear(512,output_size)
    ...
    def forward(self,observation):
        ...
        observation = self.fc(observation.view(observation.size(0), -1))
        v = self.value(observation)
        a = self.advantage(observation)
...
class Agent_DQN(Agent):
    ...
    def optimize_model(self):
        ...
        chosen_action =  self.policy_net(next_state_batch).max(1)[1].view(-1, 1)
        next_q = reward_batch + (1 - done_batch) * GAMMA * self.target_net(next_state_batch).gather(1,chosen_action).detach().squeeze(1)
