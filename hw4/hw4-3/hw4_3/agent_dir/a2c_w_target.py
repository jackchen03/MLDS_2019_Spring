...
class Agent_A2C(Agent):
    def __init__(self, env, args):
        ...
        self.A2C_target = Actor_Critic(self.n_actions).to(device)
        ...
        if args.test_a2c:
            ...
            print(self.checkpoint['latest_reward'])
    ...
    def train(self):
        ...
        self.A2C_target.load_state_dict(self.A2C.state_dict())
        self.A2C_target.eval()
        ...
        for i_episode in range(num_episodes):
            ...
            while not done:
                ...
                _, next_value_target = self.A2C_target(next_state)
                ...
                advantage.append(next_value + reward - value)
                closs.append(next_value_target + reward - value)
            ...
            closs = torch.cat(closs)
            dist_entropy = -(log_probs * probs).sum(-1).mean()

            actor_loss = -(log_probs * advantage.detach()).mean()
            self.optimizer.zero_grad()
            (actor_loss - 0.01 * dist_entropy).backward(retain_graph = True)
            self.optimizer.step()

            critic_loss = closs.pow(2).mean()
            self.optimizer.zero_grad()
